//G.Kindermann added harvesting losses variable 09.2010
//M.Gusti added functions getStockingdegree() and getRotationPeriod 09.2010
// AgeStruct modifications (13.10.2009):
// - copy constructor added;
// - 2 new methods: getAgeClasses, getActiveAge;
// Assignment operator overloading; A.Bun: added 16.08.2010 to allow copying objects
// Correction of the problem with negative biomass (getBm()), G.Kindermann 31.11.2010
// MG: 25.01.2011 corect problem with negative biomass (getBm(age))
#include "ageStruct.h"

namespace g4m {

  ageStruct::v::v() {
    area=0.; enSw=0.; enRw=0.; vnSw=0.; vnRw=0.; dbEn=0.; dbVn=0.; hv=0.; enSwT=0.; enRwT=0.; vnSwT=0.; vnRwT=0.; hvT=0.;
  }
  ...
  ageStruct::v ageStruct::deforest(double area, int type) {
	...
	double mul = 1. - area/totArea;
	       mul = mul < 0. ? 0. : mul;// Correction of the problem with negative biomass, G.Kindermann 31.11.2010
           mul = mul > 1. ? 1. : mul;
...
//	  ret.enRw += totalWood * hle.v(dat[i].d+id) - totalWood * sws.v(dat[i].d+id);
      double sawnWood = sws.v(dat[i].d+id); //Possible Sawn wood
      double restWood = hle.v(dat[i].d+id) - sawnWood;
      ret.enSw += totalWood * sawnWood;
      ret.enRw +=totalWood * restWood;
//          ret.hv += totalWood * (1. - sawnWood);
//          ret.hv += totalWood * (1. - restWood);
      ret.hv += totalWood * (1. - sawnWood - restWood);	  
...
//MG
  double ageStruct::getStockingdegree() {
    return(targetSd);
  }
  
  double ageStruct::getRotPeriod() {
...
//          ret.hv += totalWood * (1. - restWood);
      ...
      ret.enSwT=ret.enSw; ret.enRwT=ret.enRw; ret.hvT=ret.hv;//MG
      divArea(ret, getArea());
    }
    return(ret);
  }
  ...
  ageStruct::v ageStruct::aging() {
    ...
    for(int i=activeAge; i>=0; --i) {
      ...
      if(targetSd > 0.) {  //With thinning
        double maxSd = 1./it->gSdopt2Sdnat(i, mai, 1.);
        double stockingDegree = targetSd;
        if(stockingDegree > maxSd) {stockingDegree = maxSd;}
        std::pair<double,double> tmp = it->gDbmt(i, mai, dat[i].bm, stockingDegree);
      ...
      if(thin > 0. && dbv.v(dat[i].d+id) * thin >= minDbVn) {
        ...
        ret.hvT += totalWood * (1. - (sawnWood + restWood));
      }
      dat[i+1] = dat[i];
    }
    ret.vnSwT=ret.vnSw; ret.vnRwT=ret.vnRw; //MG
    double area=getArea();
    if(area > 0.) {ret.vnSw /= area; ret.vnRw /= area; ret.dbVn /= area; ret.hv = ret.hvT/area;}
  ...
  (int rotationPeriod, double area, double sd) {
    ...
    for(int i=0; i<rotationPeriod; ++i) {
      if(targetSd > 0.) {
        ...
        double stockingDegree = targetSd;
        if(stockingDegree > maxSd) {stockingDegree = maxSd;}
        tmp.bm = it->gHbmt(i, mai) * stockingDegree;
        std::pair<double,double> tt = it->gDbmt(i, mai, tmp.bm, stockingDegree);
        dbm=tt.first; thin=tt.second;
        tmp.d =  it->gD(i, mai, stockingDegree);
        tmp.h =  it->gH(i, mai);
      }
      else {
...
//    double ret = -1.;
    double ret = 0.;      //MG: 25.01.2011 corect problem with negative biomass
    if(age < ageClasses) {
      ret = dat[age].bm;
    }
    return(ret);
  ...
  (incrementTab *ait, ipol<double,double> &asws
   ...
   , double aarea, double aminRot, bool ageClassesFlag
   , int aageclasses)
  {
    activeAge = 0;
    it = ait;
...
ageStruct::ageStruct(const ageStruct & g) {
    ...
    dat = new cohort[ageClasses+1];
    memcpy(dat, g.dat, (ageClasses+1) * sizeof(cohort));
...
// Assignment operator overloading; A.Bun: added 16.08.2010 to allow copying objects
ageStruct & ageStruct::operator = (const ageStruct & g)
 {
  if (this != &g) {
    it = g.it;
    ageClasses = g.ageClasses;
