...
extern countryData<double> CountriesNforCover;
extern countryData<double> CountriesNforTotC;
extern countryData<double> CountriesAfforHaYear;  
extern countryData<double> CountriesAfforCYear;  
extern countryData<double> CountriesAfforCYear_ab;  
extern countryData<double> CountriesAfforCYear_bl;  
extern countryData<double> CountriesAfforCYear_biom;  
extern countryData<double> CountriesAfforCYear_dom;    
extern countryData<double> CountriesAfforCYear_soil;    
extern countryData<double> CountriesOforCover;
extern countryData<double> CountriesDeforHaYear;  
extern countryData<double> CountriesOfor_abC;
extern countryData<double> CountriesOforC_biom; 
extern countryData<double> CountriesDeforCYear;  
extern countryData<double> CountriesDeforCYear_bl;   
extern countryData<double> CountriesDeforCYear_ab; 
extern countryData<double> CountriesDeforCYear_biom;
extern countryData<double> CountriesDeforCYear_dom;   
extern countryData<double> CountriesDeforCYear_soil;     
extern countryData<double> CountriesWoodHarvestM3Year;    
extern countryData<double> CountriesWoodHarvestPlusM3Year; 
extern countryData<double> CountriesWoodHarvestFmM3Year;
extern countryData<double> CountriesWoodHarvestDfM3Year;
extern countryData<double> CountriesWoodLoosCYear;   
extern countryData<double> CountriesHarvLossesYear;
extern countryData<double> CountriesManagedForHa;     
extern countryData<double> CountriesManagedCount;  
extern countryData<double> CountriesMAI;    
extern countryData<double> CountriesCAI;    
extern countryData<double> CountriesCAI_new;      
extern countryData<double> CountriesFM;   
extern countryData<double> CountriesFMbm;     
extern countryData<double> CountryRotation; 
extern countryData<double> CountriesWprod;
extern countryData<double> CountriesProfit;
extern countryData<double> CountryregWoodHarvestM3Year; 
extern countryData<double> CountryregWoodHarvestFmM3Year;
extern countryData<double> CountryregWoodHarvestDfM3Year;  
extern countryData<double> CountryregWprod;
extern countryData<double> CountryregRotation; 
          ...
          griddata2<double> &harvestGrid, int year, double priceC, int asID, griddata2<double> &OforestShGrid)
{
	...
	char prstr[5];
	string regstr = IntToStr((int)it.POLESREG.v());
	//  if (PriceCi!=0) {
	//    int2str(PriceC,prstr);
	//    regprice = "re"+string(regstr)+"price"+ string(prstr);
	//  } else 
	{
		regprice = "re"+regstr+"price0";
	}
	regprice0 = "re"+regstr+"price0";
	//  {
	...
	double harvMAI = maiForest.get(xi,yi)*it.FTIMBER.v()*(1-coeff.HarvLoos.v());
	...
	if(it.AGRSUIT.v(year) > 0 && OforestShare > 0)
	{
		...
		double pDefIncome = it.CABOVEHA.v() *           
		...
		double sDefIncome = it.CABOVEHA.v() *              
			(TimberPrice * it.FTIMBER.v()
			* (1. -coeff.HarvLoos.v())
			- coeff.PriceC.v(year));
...
//MG: Afforestation
   if (OforestShare + AforestShare < maxfor){
      if ((it.POTVEG.v())<9 && (it.NPP.v())>0)  { // MG: We afforest only places, where potential vegetation is forest    
...
//  PlantPhytHaBmGr = newCohort.getBm();
  ...
  double CurPlantPhytBmGr=(PlantPhytHaBmGr-singleCell.prevPlantPhytHaBmGr) * singleCell.LandAreaHa;
  double CurPlantPhytBlGr=(PlantPhytHaBlGr-singleCell.prevPlantPhytHaBlGr) * singleCell.LandAreaHa;
...
double harvestDfM3Ha = (defBiomass-resDefor.hv)*singleCell.deforestA[Age]*(1.-it.SLASHBURN.v()) * it.FTIMBER.v(); // wood obtained at deforestation
double deforWoodTotM3 = harvestDfM3Ha * singleCell.LandAreaHa;
double harvestFmTotM3 = (harvWood*OforestShare+harvWoodNew*singleCell.AforestSharePrev)*singleCell.LandAreaHa;
double harvestTotM3 = harvestFmTotM3 + deforWoodTotM3;
...
double harvestFcM3 = (sawnW + restW + sawnWnew + restWnew)* it.FTIMBER.v(); // current harvested wood from final cut in the cell, m3/ha
double harvestThM3 =(sawnThW + restThW + sawnThWnew + restThWnew) * it.FTIMBER.v(); // current harvested wood from thinning in the cell, m3/ha
double harvestTotPlusM3 = (harvWoodPlus*OforestShare + harvWoodNewPlus*singleCell.AforestSharePrev+(defBiomass-(1-resUse)*resDefor.hv) * (1. - it.SLASHBURN.v())*singleCell.deforestA[Age]* it.FTIMBER.v())*singleCell.LandAreaHa; //Total current harvested wood in the cell, m3, including residues (harvest losses)
//double CAI_m3ha = (abBiomassO-singleCell.ObiomassPrev + harvWoodLost + sawnW + restW + sawnThW + restThW + harvL) * it.FTIMBER"].v(); //Current annual increment m3/ha
//double CAI_m3ha = (abBiomassO-singleCell.ObiomassPrev + sawnW + restW + sawnThW + restThW + harvL) * it.FTIMBER.v(); //Current annual increment m3/ha
//double CAI_m3ha = (abBiomassO-singleCell.ObiomassPrev*(1-singleCell.deforestA[Age]) + sawnW + restW + sawnThW + restThW + harvL + harvWoodLost) * it.FTIMBER.v(); //Current annual increment m3/ha
double CAI_m3ha = ((abBiomassO-singleCell.ObiomassPrev) + sawnW + restW + sawnThW + restThW + harvL + harvWoodLost) * it.FTIMBER.v(); //Current annual increment m3/ha
CAI_m3ha = CAI_m3ha < 0. ? 0. : CAI_m3ha;
double phytNewIncr = 0.; if (AforestShare > 0) {phytNewIncr = CurPlantPhytBmGr/(AforestShare * singleCell.LandAreaHa);}
double CAI_new_m3ha = (phytNewIncr + harvWoodLostNew + sawnWnew + sawnThWnew + restThWnew + restWnew + harvLNew) * it.FTIMBER.v();
...
//double FMsink_ab = (abBiomassO-singleCell.ObiomassPrev*(1-singleCell.deforestA[Age]))*3.6666666667*unitConv;
double FMsink_ab = (abBiomassO-singleCell.ObiomassPrev)*3.6666666667*unitConv;
double FMsink_bm = FMsink_ab * (1+coeffBl);
...
//    fff<<"\t"<<EmissionsCur*3.6666666667*unitConv<<"\t"<<defBiomass*deforestHa*3.6666666667*unitConv<<"\t"<<it.CBELOWHA.v()*deforestHa*3.6666666667*unitConv;
//    fff<<"\t"<<(defBiomass+it.CBELOWHA.v())*deforestHa*3.6666666667*unitConv<<"\t"<<(EmissionsDeadBurnCur+EmissionsLitterCur)*3.6666666667*unitConv;
//    fff<<"\t"<<EmissionsSOCCur*3.6666666667*unitConv;
//    fff<<"\t"<<EmissionsAfforCur*3.6666666667*unitConv<<"\t"<<CurPlantPhytHaBmGr*3.6666666667*unitConv<<"\t"<<CurPlantPhytHaBlGr*3.6666666667*unitConv;
//    fff<<"\t"<<(CurPlantPhytHaBmGr+CurPlantPhytHaBlGr)*3.6666666667*unitConv<<"\t"<<EmissionsLitterAfforCur*3.6666666667*unitConv<<"\t"<<EmissionsSOCAfforCur*3.6666666667*unitConv;
//    fff<<endl;
}
...
// Output results for countries
     ...
     CountriesAfforCYear.inc(Country,year,EmissionsAfforCur*3.6666666667*unitConv);  
     CountriesAfforCYear_ab.inc(Country,year, CurPlantPhytBmGr*3.6666666667*unitConv);  
     CountriesAfforCYear_bl.inc(Country,year, CurPlantPhytBlGr*3.6666666667*unitConv);   
     double EmAffBm = (CurPlantPhytBmGr+CurPlantPhytBlGr)*3.6666666667*unitConv; // CO2 sink in forest living biomass of planted forest, mtCO2/year
     CountriesAfforCYear_biom.inc(Country,year, EmAffBm);
     CountriesAfforCYear_dom.inc(Country,year, EmissionsLitterAfforCur*3.6666666667*unitConv);    
     CountriesAfforCYear_soil.inc(Country,year, EmissionsSOCAfforCur*3.6666666667*unitConv);     
 //--------- 
     ...
     CountriesDeforCYear.inc(Country,year,EmissionsCur*3.6666666667*unitConv);    
     CountriesDeforCYear_ab.inc(Country,year, defBiomass*deforestHa*3.6666666667*unitConv);
     CountriesDeforCYear_bl.inc(Country,year,it.CBELOWHA.v()*deforestHa*3.6666666667*unitConv);      
     double EmDefBm = (defBiomass+it.CBELOWHA.v())*3.6666666667*unitConv; // CO2 lost from living biomass of old forests due to deforestation, mtCO2/hayear
     CountriesDeforCYear_biom.inc(Country,year,EmDefBm*deforestHa);       
     CountriesDeforCYear_dom.inc(Country,year, (EmissionsDeadBurnCur+EmissionsLitterCur)*3.6666666667*unitConv);     
     CountriesDeforCYear_soil.inc(Country,year, EmissionsSOCCur*3.6666666667*unitConv);     
//--------- 
...
//     CountriesCAI_new.inc(Country,year,(phytNewIncr + sawnWnew + sawnThWnew + restThWnew + restWnew + harvLNew) * it.FTIMBER.v()*singleCell.AforestSharePrev*singleCell.LandAreaHa);     
	...
	double profit = harvestTotM3 * TimberPrice - decision.plantingCosts() * cohort.getArea(0) * OforestShare * singleCell.LandAreaHa;
...
//cout<<Country<<endl;
    ...
    string countryregstr = "re"+IntToStr(countryreg)+"price0";
...
//}
    CountryregWprod.setVal(countryreg,year,wprod[countryregstr].v(year));
...
//singleCell.FMsink = (abBiomassO-singleCell.ObiomassPrev)*OforestShare*singleCell.LandAreaHa*3.6666666667*unitConv; // FM sink in GgCO2/yr
